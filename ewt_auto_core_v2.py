...
class Ewt_upload_progress_v2:
    ...
    def run_j(self,cls,day,rate):
        lesson_id=cls["contentId"]
        course_id=cls["parentContentId"]
        lesson_duration = cls["duration"]
        if cls['ratio']>(rate-0.003):
            #residue_time_ms = lesson_duration*1000*(1-cls['ratio'])
            return
        else:
            residue_time_ms = round((lesson_duration*1000*rate - lesson_duration*1000*cls['ratio'])/2)
        
        z_times = math.floor((residue_time_ms/1000)/60)
        ...
        for i in range(1,z_times+2):
            report_time  = begin_time + (i-1)*60000
            threading.Thread(target=uploadprogress_v3_j, args=(self.task_id,self.account,self.userid,self.token,secret,x_bfe_session_id,begin_time,report_time,60000,lesson_id,course_id,i,self.ip)).start()
            time.sleep(60.3)

        #补充剩下不足60s的部分
        duration = round(residue_time_ms-(60000*z_times))
        report_time = report_time + duration
        threading.Thread(target=uploadprogress_v3_j, args=(self.task_id,self.account,self.userid,self.token,secret,x_bfe_session_id,begin_time,report_time,duration,lesson_id,course_id,z_times+2,self.ip)).start()
    ...
    def start_run(self):
        ...
        rates = [0.333,0.588,0.555,0.666,0.333,0.666,0.777,0.222]
        s=0
        s_reached = 0
        self.login()
        self.getuserinfo()
                ...
                for day in self.getdaylist(ho,sceneid):
                    if day['day'] > getTime0ClockOfday(self.task_advance_day):
                        if s == 0 and s_reached == 0:
                            ...
                            upload_log.uploadlog(self.task_id,self.account,1,200,f"OK {s} video finished. {s_reached} video reached. Advanced {self.task_advance_day} days")
                    ...
                    for cls in self.gethomeworklist(ho,day,sceneid):
                        if int(cls['subjectId']) in self.subject_filter:
                            continue
                        rate = rates[(int(cls["contentId"])+int(self.userid))%8]
                        if cls['ratio']>(rate-0.003):
                            logging.info(f'[2203][account:{self.account}],LESSON REACHED, [title:{cls["title"]},lessonid:{cls["contentId"]},courseid:{cls["parentContentId"]}]')
                            upload_log.uploadlog(self.task_id,self.account,0,2203,f'LESSON REACHED, [title:{cls["title"]},lessonid:{cls["contentId"]},courseid:{cls["parentContentId"]},ratio:{round(cls["ratio"]*100,1)}%]')
                            s_reached += 1
                            continue
                        if cls["contentType"]==1:
                            self.run_j(cls,day,rate)
                            s+=1
                            #if(s%5==0): print("s====",s)
        time.sleep(2)
        upload_log.uploadlog(self.task_id,self.account,1,200,f"OK {s} video finished. {s_reached} video reached. Advanced {self.task_advance_day} days")
...
# 对应 SDK3.0.7
def uploadprogress_v3_j(task_id,account,userid,token,secret,x_bfe_session_id,begin_time,report_time,duration,lesson_id,course_id,i,ip):
    if i==1:
        action = 1
    else:
        action = 2
    ...
    payload = {
        ...
        [{
            ...
            "stay_time": 0 if action==1 else duration,
            "status": 1,
    ...
    signature_str = f'action={action}&duration={0 if action==1 else duration}&mstid={token}&signatureMethod=HMAC-SHA1&signatureVersion=1.0&timestamp={report_time}&version=2022-08-02'
    ...
    try:
        ...
        upload_log.uploadlog(task_id,account,10,5011,f'Uplaod_progress Error,[lessonid:{lesson_id},courseid:{course_id},index:{i},begin_time:{begin_time},report_time:{report_time},duarion:{duration}], {Response.text}')